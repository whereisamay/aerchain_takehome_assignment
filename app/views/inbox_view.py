...
def _attachments(row: dict, key: str) -> None:
...
def _rfq_index(ids: list[str]) -> int:
    return ids.index(DEMO_RFQ) if DEMO_RFQ in ids else 0


def _unmatched(names: dict) -> None:
    ...
    for r in rows:
        with st.container(border=True):
            ...
            st.text(r["body"])
            _attachments(r, f"un{r['id']}")
            c1, c2, c3 = st.columns([2, 2, 1])
            rid = c1.selectbox("RFQ", rfq_ids, key=f"as{r['id']}_r", index=_rfq_index(rfq_ids))
            vc = c2.selectbox("Vendor", list(names), key=f"as{r['id']}_v", index=None,
                              format_func=lambda c: f"{c} · {names[c]}", placeholder="Who sent this?")
            c3.write("")
...
def _upload(names: dict) -> None:
    ...
    with st.form("upload", clear_on_submit=True):
        ...
        rid = c2.selectbox("For RFQ", rfq_ids, index=_rfq_index(rfq_ids))
...
def render() -> None:
    st.header("Vendor Inbox")
    transport_note()
    ensure_rfqs()
    if msg := st.session_state.pop("inbox_toast", None):
        st.toast(msg)
    names = vendor_names()
    ...
    rows = mail.inbox()
    if not rows:
        st.info("Inbox is empty. Send the RFQ from the RFQ Sender, then simulate the replies — or upload a file below.")
    else:
        st.dataframe(pd.DataFrame([{
            "Status": {"matched": "✅ matched", "assigned": "🟦 assigned"}.get(r["status"], "⚠️ unmatched"),
        ...
        for r in rows:
            with st.expander(f"{r['vendor_code'] or '?'} · {r['from_addr']} · {r['subject']}"):
                st.caption(f"How it was matched: {r['match_detail']}")
                st.text(r["body"])
                _attachments(r, f"in{r['id']}")

    with st.expander("Upload a response", expanded=not rows):
        _upload(names)
